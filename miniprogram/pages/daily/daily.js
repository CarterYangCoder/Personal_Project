...
const { dailyPreview } = require('../../utils/mock')
...
Page({
  data: {
    ...
    diaries: [], diaryGroups: [], displayDiaries: dailyPreview, dailyPreview, todayCount: 0, importantCount: 0,
    moods: ['平静', '开心', '感动', '疲惫', '放空'], moodIndex: 0,
    diaryDate: '', diaryTime: '', diaryCategory: '', diaryTitle: '', diaryContent: '', diaryImportant: false,
    editingId: '', scrollTarget: '', favoriteSongs: [], isCurrentFavorite: false, showComposer: false
  },
  ...
  updateDiaries(records) {
    ...
    this.setData({ diaries, diaryGroups: Array.from(groupMap.values()), displayDiaries: diaries.length ? diaries.slice(0, 3) : dailyPreview, todayCount: diaries.filter(item => item.date === formatDate()).length, importantCount: diaries.filter(item => item.important).length })
  },
  async syncRecords() {
  ...
  playSong() {
    wx.showToast({ title: '已为你播放今日一歌', icon: 'none' })
  },
  nextSong() {
  ...
  selectMood(e) { this.setData({ moodIndex: Number(e.currentTarget.dataset.index) }) },
  ...
  editDiary(e) {
    ...
    this.setData({
      ...
      moodIndex: Math.max(0, this.data.moods.indexOf(diary.mood)), scrollTarget: '', showComposer: true
